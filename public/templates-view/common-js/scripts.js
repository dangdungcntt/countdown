...
(function ($) {
	...
	if(isExists('#classy-countdown')){
		var now = $.now();
		$('#classy-countdown').ClassyCountdown({
			theme: "flat-colors-very-wide",
			end: now + ((new Date($('#classy-countdown').data('date'))).getTime() - now) / 1000
		});
	}
	...
	if(isExists('#normal-countdown')){
		var date = new Date($('#normal-countdown').data('date'));
